...
$$s_{stringy} = \frac{|V^{(2)}|}{|V|-|V^{(1)}|}$$  
 This formulation corresponds to the later and commonly used definition of stringy introduced in @wilkinson2006high. An earlier definition was proposed in @scag, where stringy was defined as the ratio between the diameter of the MST and its total edge length:
...
scagissues_tb <- tibble(Scagnostic = c("Striated", 
                      ...
                      Issues = c("The striated measure can identify when one variable is discrete, and one is continuous, but is unable to identify two discrete variables. Additionally, since the edges used to calculate the striated measure are a subset of those used by the stringy measure, the two scagnostics are highly correlated, and often identify the same plots.",
                               ...
                               "This measure can identify skewed edge lengths, such as the L shape in the visual table, that typically require a log transformation. The skewed value rarely drops below 0.5 or rises above 0.8. Skewed seems to suffers from a similar issue to sparse regarding binning.",
...
scagfuncs_tb <- tibble(dt = c("scree", 
                      ...
                      text = c("Generates a scree object that contains the Delaunay triangulation of the scater plot.",
                               ...
                               "Compute adjusted clumpy scagnostic measure.",
                               ...
                               "Compute the original striated scagnostic measure.",
...
Figure `r knitr::asis_output(ifelse(knitr::is_latex_output(), '\\@ref(fig:wbistatic)','\\@ref(fig:wbiinteractive)'))` (left plot) shows a summary of the top scagnostic values. This is displayed as a vertically-oriented side-by-side dotplot. Missing from the plot, but not the calculations, is that splines was not the high for any pair of variables in the data set. The other two plots show the pairs of variables with high values on outlying.
