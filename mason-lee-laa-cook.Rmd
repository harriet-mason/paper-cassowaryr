...
To pick out which pair wise plots might be interesting to examine, we use a SPLOM of scagnostic values. Technically, any high-dimensional visualisation approach, such as a parallel co-ordinate plot, can be utilised to find interesting pairs, but we opted to use a SPLOM as it is the approach taken by @scagdist. Outliers in the SPLOM are distinct from other scatter plots made using the same data, and indicate that a plot is worth further inspection. 
