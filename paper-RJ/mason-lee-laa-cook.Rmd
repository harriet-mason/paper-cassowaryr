...
```{r,  Striated Comparison, fig.height=8, fig.width=10}
# Make Visual Table
# Data
...
#### Clumpy Adjusted
There is a robust version of clumpy that is defined in the paper *Improving the robustness of scagnsotics* [@robust], and has been included in the package. This measure is stronger than the original clumpy measure as it builds a cluster hierarchy and considers a large number of edges. Ultimately it seems to suffer from similar issues to the original version of the scagnostic, and takes a significantly longer time to compute than the original clumpy measure. It is not useful in large scale scagnostic calculations (such as those done on every pairwise combination of variables as is intended by the package) so is not discussed here. The issues that need to be addressed with the new clumpy measure are:
...
```{r, AFLW Scatter Plots, include=FALSE, eval=FALSE, echo=FALSE, fig.height=5, fig.width=9, out.width = "95%", fig.align='center'}
load("data/aflw.rda")
...
```{r, AFL relevent SPLOMS, include= FALSE, eval=FALSE, echo=FALSE, fig.height=3, fig.width=9, out.width = "100%", fig.align='center'}
test <- AFLW_scags %>%
...
### Team Comparison
One use of scagnostics is to compare between group differences in the shape of scatter plots. Here, we will apply the scagnsotics to the best and worst team in the 2020 AFLW season, North Melbourne and Richmond respectively. Looking at the *shape* differences on scatter plots between the two teams can give us a new method to understand what differentiates a good team from a bad one. 
...
After averaging each players statistics across all the games in the season, there are 3 scatter plots that stand out as having a difference in the shapes between the two teams. They are:
  
1. Turnovers vs Disposal Efficiency
  
2. One Percenters vs Inside 50s
  
3. Clangers vs Handballs
  
We will analyse the shape difference in these scatter plots here.

#### Turnovers vs Disposal Efficiency
...
This scatter plot was identified as interesting as the two teams has a large difference on the clumpy_adjusted and convex measures. A good player will have a high disposalEfficiency (they rarely kick it to the other team) and a low number of turnovers (they rarely kick it somewhere no player can get it) and we would expect a linear relationship between the two, as causing a both is likely caused by bad play. A comparison of these plots shows the best team in the league, North Melbourne, adheres pretty well to this linear relation, while Richmond does not. That indicates that Richmond players either overwhemingly kick the ball into space to avoid kicking to the other team, or they overwhelmingly give a risky pass that is likely to go to the other team. They do not evenly choose between the two.

#### One Percenters vs Inside 50s
