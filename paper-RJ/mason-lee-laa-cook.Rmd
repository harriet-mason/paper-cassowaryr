...
abstract: >
  Data sets that have a large number of numeric vairbales, also have an arbitrarily large number of possible pairwise scatter plots. Looking at them all is infeasible for any individual, and so in order give priority to the most interesting plots, we have designed the "cassowaryr" package that allows users to sort scatter plots based on a list of Scagnsotics. Scagnsotics are scatter plot diagnostics that give numerical summaries to the shapes of scatter plots. These scagnsotics are shown to correctly order simulated data that represent scatter plot shapes a user would find interesting. They are also applied to examples in macro and micro time series, physics and sports data where the package succeeds in picking out interesting scatter plots from a large collection.
draft: true
author:  
...
```{r, Table of Plots, fig.height=10, fig.width=10}
...
While the scagnsotics can be shown to correctly sort scatter plots that are all visually interesting and unique, is is entirely different for the measures to correctly identify interesting visual features from non-simulated data. Assessing the scagnsotics on this test motivates the examples below.
  
With these issues in mind, we have defines and written several new scagnsotics to try that should still work even with the removal of binning as a pre-processing step. 
...
```{r,  Striated Comparison, fig.height=5, fig.width=10}
# Make Visual Table
# Data
...
While these two measures may seem similar at a glace, there are a few minor things that make the striated_adjusted scagnsotic an improvement on the stirated scagnsotic. First of all, the perfect 1 value on striated goes to the "line" scatter plot. While this does fulfil the definition, it is not what the measure is supposed to be looking for, rather supposed to be identifying the "vlines" scatter plot. Since striated does not count the right angles that go between the vertical lines, a truely striated plot will never get a full 1 on this measure, striated adjusted fixes this. After that there is a large gap in both measures because none of the other scatter plots have a strictly discrete measure on the x or y axis. The lower plots show that striated_adjusted is also better at identifying discrete relationships with a rotation and noise added as shown in the "discrete" plot. In striated "discrete" is lower in the order than "outlying" which would indicate that striated has finished looking at discreteness. In striated_adjusted, after the plots with strict discreteness in "vlines" or strict rotated disceteness in "line", is the noisey and rotated "discrete" plot. Therefore in terms of ordering the plots in how well they represent the feature of discreteness, striated_adjusted outperforms striated.


#### Clumpy Adjusted
There is a robust version of clumpy that is defined in the paper *Improving the robustness of scagnsotics*, and has been included in the package. This measure is stronger than the original clumpy measure as it builds a cluster hierarchy and considers a large number of edges. Ultimately it seems to suffer from similar issues to the original version of the scagnsotic, and takes a significantly longer time to compute than the original scagnostic. It is not useful in large scale scagnostic calculations (such as those done on every pairwise combination of variables as is intended by the package) so is not discussed here. The issues that need to adressed with the new clumpy measure are:

1. It needs to consider more than 1 edge in its final measure to prevent large changes from outliers and make the measure more robust

2. The clusters need to be weighted by the size of their clusters so the measure does not simply identify outliers

3. It should not consider vertices thats adjacent angles for a straight line (to avoid identifying the angles striated identifies)

To easily identify the edges that sit between clusters, we sort the edges and calculate the difference and take the maximum. The largest gap will indicate the jump between the edges separating clusters and the edges within clusters. The clumpy_adjusted measure is the ratio between the mean of the large edges and the maximum of the small edges. By taking the largest of the small edges, if the gap is not large

*To add in after in package:* One we have identifies the large edges, we remove them from the MST and identify which cluster all the inter cluster edges belong to. From each cluster we identify the largest edge (to avoid the measure being arbitrarily from small edges in the MST) and calculate the ratio between the largest inter cluster edge, and the large edge that connects the clusters. These ratios are added together as a weighted mean, where each weight is determined by the number of points that do not satisfy the 180 degree striated requirement. This weighted mean is the final clumpy_adjusted value.

```{r,  Clumpy Comparison, fig.height=5, fig.width=10}
...
Even with a partially completed clumpy_adjusted measure we can see the improvement on the clumpy measure. The first obvious outcome is that the measure is more spread out, and so values range more accurately from 0 to 1. More importantly the measures do a better job of ordering the scatter plots. On the original clumpy measure the "clusters" scatter plot was next to last, so in terms of using it to identify clusters, the measure is about as good as one that is randomly generated. On the clumpy_adjsuted measure "clusters" is the third highest scatter plot, so it does a much better job of identifying it when considering the order of the scagnostics. The clumpy_adjusted measure only has the "vlines" and the "outliers" scatter plots ahead "clusters" due to the measure not yet penalizing plots with adjacent straight lines or small lone clusters that appear in those two plots. With these adjustments the clumpy_adjusted measure should work correctly without the binning pre-processing step.
...
When we try and compare the best and the worst AFLW teams  

- **turnovers vs disposalEfficiency**: This scatter plot was identified as interesting as the two teams has a large difference on the clumpy_adjusted and convex measures. A good player will have a high disposalEfficiency (they rarely kick it to the other team) and a low number of turnovers (they rarely kick it somewhere no player can get it) and we would expect a linear relationship between the two, as causing a both is likely caused by bad play. A comparison of these plots shows the best team in the league, North Melbourne, adheres pretty well to this linear relation, while Richmond does not. That indicates that Richmond players either overwhemingly kick the ball into space to avoid kicking to the other team, or they overwhelmingly give a risky pass that is likely to go to the other team. They do not evenly choose between the two.
  
- **inside50s  vs onePercenters**: This scatter plot was also identified as interesting by the two teams having a large difference on the clumpy_adjusted and convex measures. If the player gets the ball inside the 50m arc around the oppositions goal they are likely a forward, if they get perform a lot of onePercenters they are likely a defensive player, if they do a bit of both they are likely a midfielder. A well structured team will have  a lower triangular shape on this scatter plot, as players are sticking to their position and excelling in that position, which is the structure North Melbourne has. Richmond has several players that score highly on both onePercenters and are frequently inside50s. This implies Richmond has a hanful of players that play every position well and are a bit all over the field, and have less specialisation in positions.

- **handballs vs clangers**: This scatter plot was identified as interesting by all the association measures. Handballs are a measure of one of the legal methods of getting rid of the ball, clangers are a measure of the illegal methods of getting rid of the ball. If the number of clangers for each player in the team linearly related to the number of handballs, it means the team as a whole is efficient at getting rid of the ball, if this scatter plot is spread out, it means there are large gaps in the efficiency of each player. This scatter plot shows us that the Richmond team has a greater difference in ability than the North Melbourne Team.
