...
```{r}
#which scatter plots
which_striated_plots <- function(scag, feature){
  pad = FALSE
  # Original Measure
  if(all(scag %in% c("striated","striated_adjusted"), feature %in% c("vlines", "discrete", "weak"))){
    pad = TRUE
  }
  #if(all(scag=="striated_adjusted", feature %in% c("vlines", "discrete", "weak"))){
  #  pad = TRUE
  #}
}
...
plot_data_striated <- plot_path_data %>%
  ...
  mutate(doplot = ifelse(all(scagnostic %in% c("striated","striated_adjusted"), 
                             feature %in% c("vlines", "discrete", "weak")),
                         TRUE,
                         FALSE)) %>% #which_striated_plots(scagnostic, feature)) %>%
  ungroup() %>%
  filter(doplot==TRUE)
...
striated_visual_table <- ggplot(plot_data_striated, aes(x=value , y=scagnostic))+
  geom_point_img(aes(img = plotad), size = 2, position_jitter(width = 0, height = 2)) + 
  xlim(-0.05,1.05) +
  scale_size_identity()+
  xlab("Value") +
  ylab("Scagnostic") +
...
ggsave("figures/striated_visual_table.png", striated_visual_table, width=20, height=6)
#visual_table
