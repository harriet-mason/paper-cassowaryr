...
- **anscombe_tidy**: Data from Anscombe's famous example in tidy format
- **datasaurus_dozen**: datasaurus_dozen data
- **datasaurus_dozen_wide**: datasaurus_dozen data
...
- **sc_convex**: Compute convex scagnostic measures
- **sc_dcor**: Distance correlation index.
- **sc_monotonic**: Measure of Spearman Correlation
- **sc_outlying**: Compute outlying scagnostic measure using MST
- **sc_skewed**: Compute skewed scagnostic measure using MST
- **sc_skinny**: Compute skinny scagnostic measure
- **sc_sparse**: Compute sparse scagnostic measure using MST
- **sc_sparse2**: Compute adjusted sparse measure using the Alpha Hull
- **sc_splines**: Spline based index.
- **sc_striated**: Compute stirated scagnostic measure using MST
- **sc_striated2**: Compute angle adjusted striated measure using MST
...
- **draw_alphahull**: Drawing the alphahull
- **draw_convexhull**: Drawing the Convex Hull
- **draw_mst**: Drawing the MST

### Summary functions
The summary functions are the perferred method for users to calculate scagnostics. 
...
```{r bbh-scags-interactive, fig.cap="Selected pairs of scagnostics computed for the black hole mergers data. Groups of parameter combinations can be seen to stand out in the left plot (high on skinny and low on convex) and in the middle plot (high on both dcor and splines). The plot on the right shows clumpy vs skewed, where we would have expected more structure, since there are several combinations that lead to a clustering of the points.", layout = "l-body", eval=knitr::is_html_output()}
...
The final plot is showing clumpy vs skewed, illustrates current limitations with the clumpy measure. For example, the selected plot time vs alpha (final plot of Figure \@ref(fig:blackholes)) takes a similar value of about 0.9 for the clumpy measure as seen for chi_p vs chi_tot, while combinations like ra vs dec score higher, around 0.95. Indeed this data contains many combinations that will break the points into groups, as can already be seen from the examples shown here, but these cannot be readily identified using the clumpy measure.

```{r blackholes, fig.cap="Features in the BBH data that stand out on several of the scagnostics measures (convey, skinny, splines and dcor), showing strong relations between variables including non-linear and non-functional dependencies. The final example (time vs alpha) is expected to take high values in clumpy, but does not stand out compared to other combinations.", fig.width = 12, fig.height = 8, out.width="100%"}
