...
Figure \@ref(fig:outlying-test-plot) shows the an example of a simulated test set, combined with the associated MST. When creating this test data set, we assumed the MST would connect via the dashed red line between points 1 and 2, but instead the MST connected via the solid green line between points 3 and 4. The difference between these choices is essentially random, both edges are the exact same length, but it has significant implications for the value returned by the outlying scagnostic.  When the MST contains the red dashed line between points 1 and 2, point 2 is identified as an internal outlier. This means both the red dashed edge, and the edge between points 1 and 2 are included in the outlying scagnostic calculation which evalutates to be $\frac{13}{29}=0.45$. When we use the actual MST (with the green edge) then point is still identified as an outlier, but the edge between points 2 and 4 is the only edge that goes into the outlying calculation. This results in a significantly smaller value on the the outlying scagnostic, which evaluates to be $\frac{4}{29}=0.14$. This shows that even scagnostics that work well without binning are susceptible to surprisingly large changes due to arbitrary decisions made by the algorithms that construct the graph objects.
...
t1 <- ggplot(tibble(x1, y1), aes(x1, y1)) + geom_point() +
  geom_text(data=tibble(x1= c(1,10,1,10,10), y1 = c(10, 10, 6,5,6), lab=c(1,2,3,5,4)), 
...
t2 <- draw_mst(x1, y1, out.rm = FALSE) + theme_classic() + 
  ...
  geom_text(data=tibble(x1= c(0.1,1,0.1,1,1), y1 = c(1, 1, 0.6,0.5,0.6), lab=c(1,2,3,5,4)), 
            aes(x=x1, y=y1, label=lab), 
