...
*totalPossessions*: The total number of disposals for a player. Possessions and disposals are the same thing.  
...
*goalAssists*: number of times the player was involved in a passage of play leading up to a goal.  
...
*scoreInvolvements*: number of times the player was involved in a passage of play leading up to a goal.  **need to work out difference between this and goalAssists**
...
```{r}
#check splines vs dcor values
scag_aflw %>%
  mutate(splines_vs_dcor = splines-dcor) %>%
  select(Var1, Var2, splines_vs_dcor) %>%
  arrange(desc(splines_vs_dcor)) %>%
  head(10)
#nothing interesting, sad.

#check striated values
AFL_stri_adj <- calc_scags_wide(aflw_num[,5:37], scags = "striated_adjusted")

ggplot2

```

```{r aflw_interactive, fig.cap="Scatterplots with high values on the splines scagnostic. Mouseover to examine the players relative the the statistics.", include=knitr::is_html_output(), eval=knitr::is_html_output()}
gs1 <- ggplotly(s1)
gs2 <- ggplotly(s2)
gs3 <- ggplotly(s3)
...
```{r aflw_static, fig.cap="Scatterplots with high values on the splines scagnostic.", include=knitr::is_latex_output(), eval=knitr::is_latex_output()}
s1 + s2 + s3
```
