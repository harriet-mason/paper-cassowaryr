...
```{r}
drawfuncs_tb <- tibble(func_name = c("draw_alphahull", 
                             "draw_convexhull",
                             "draw_mst"), 
...
```{r}
sumfuncs_tb <- tibble(func_name = c("calc_scags",
                             "calc_scags_wide"), 
...
```{r outlying-test-plot, width = 100, height = 50, out.width = "100%", fig.cap = "Plots used for testing the 'outlying' scagnostic."}
...
t2 <- draw_mst(x1, y1) + theme_classic() + 
  ...
  geom_text(data=tibble(x1= c(1,1,1), y1 = c(1, 0.7, 0.5), lab=c(1,2,3)), 
            aes(x=x1, y=y1, label=lab), 
            colour="blue", nudge_x=-0.03, nudge_y=0.03) +
  theme(legend.position="none")
...
```{r, threeplot-interactive, out.width="80%", include=knitr::is_html_output(), eval=knitr::is_html_output()}
...
```{r threeplot-static, out.width="80%", include=knitr::is_latex_output(), eval=knitr::is_latex_output(), fig.align='center'}
...
>>>>>>> d0dc828431fcbde2d1ff2c40f3c849ebc83328cd
