...
There are two scagnostics packages that compute the measures defined by @scag, `scagnostics` [@scagdist] and the archived package `binostics` [@binostics]. Both packages are based on the original C++ code written by @scagdist and since it is written in C++ the work difficult to read, debug or extend upon. The two additional association measures discussed by @Grimm were in the package `mbgraphic`, but that has also been archived by CRAN. Thus there is a need for a new implementation that enables better diagnosis of the scagnostics, and better graphical tools for examining the results. 

The paper is organised as follows. The next section introduces the scagnostics, explains how they are calculated, and develops several new measures. This is followed by a summary of the implementation in the R package, `cassowaryr`. Finally, the examples section applies scagnostics to four different data sets to illustrate their use in three different tasks. The Australian Football League Women's (AFLW) data, and a simulated binary black hole (BBH) merger event show the scagnostics value as a tool in exploratory data analysis. The collection of time series examples, using macroeconomic and microeconomic data, illustrate the scagnostics ability to differentiate groups by shape.  The world bank indicators (WBI) shows how scagnostics can be used to summarise the shape of multivariate data.
