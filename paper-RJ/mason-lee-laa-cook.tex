...
\abstract{%
Data sets that have a large number of numeric variables, have an even
larger number of possible pairwise scatter plots. Looking at them all is
infeasible for any individual, and so in order give priority to the most
interesting plots, we have designed the ``cassowaryr'' package that
allows users to sort scatter plots based on a list of Scagnsotics.
Scagnsotics are scatter plot diagnostics that give numerical summaries
to the shapes of scatter plots. These scagnsotics are shown to correctly
order simulated data that represent scatter plot shapes a user would
find interesting. They are also applied to examples in macro and micro
time series, physics and sports data where the package succeeds in
picking out interesting scatter plots from a large collection.
}
