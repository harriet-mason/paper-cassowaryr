...
\tightlist
\item
  \textbf{features}: Simulated data with special features
\item
  \textbf{anscombe\_tidy}: Data from Anscombe's famous example in tidy
  format
\item
  \textbf{datasaurus\_dozen}: datasaurus\_dozen data
\item
  \textbf{datasaurus\_dozen\_wide}: datasaurus\_dozen data
\item
  \textbf{numbat}: A toy data set with a numbat shape hidden among noise
  variables
\item
  \textbf{pk}: Parkinsons data from UCI machine learning archive
\end{itemize}
...
are: - \textbf{scree}: Pre-processing to generate scagnostic measures -
\textbf{sc\_clumpy}: Compute clumpy scagnostic measure using MST. -
\textbf{sc\_clumpy2}: Compute adjusted clumpy measure using MST -
\textbf{sc\_clumpy\_r}: Compute robust clumpy scagnostic measure using
MST - \textbf{sc\_convex}: Compute convex scagnostic measures -
\textbf{sc\_dcor}: Distance correlation index. - \textbf{sc\_monotonic}:
Measure of Spearman Correlation - \textbf{sc\_outlying}: Compute
outlying scagnostic measure using MST - \textbf{sc\_skewed}: Compute
skewed scagnostic measure using MST - \textbf{sc\_skinny}: Compute
skinny scagnostic measure - \textbf{sc\_sparse}: Compute sparse
scagnostic measure using MST - \textbf{sc\_sparse2}: Compute adjusted
sparse measure using the Alpha Hull - \textbf{sc\_splines}: Spline based
index. - \textbf{sc\_striated}: Compute stirated scagnostic measure
using MST - \textbf{sc\_striated2}: Compute angle adjusted striated
measure using MST - \textbf{sc\_stringy}: Compute stringy scagnostic
measure using MST - \textbf{sc\_striped}: Measure of Discreteness
...
are:\\
- \textbf{draw\_alphahull}: Drawing the alphahull -
\textbf{draw\_convexhull}: Drawing the Convex Hull - \textbf{draw\_mst}:
Drawing the MST
...
scagnostics. - \textbf{calc\_scags}: Compute selected scagnostics on
subsets\\
- \textbf{calc\_scags\_wide}: Compute scagnostics on all possible
scatter plots for the given data
