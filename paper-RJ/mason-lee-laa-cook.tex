...
with known interesting visual features. Following that the fine details
of the package, including function explanations and tests are discussed.
Finally the package is applied to data from the Australian Football
League Women's (AFLW) and a simulated binary black hole (BBH) merger
event to show its value as a tep in exploratory data analysis;
macroeconomic and microeconomic data to show its ability to
differentiate groups by shape; and finally the world bank indicators
(WBI) to show how the package can be used to summarise the shape of an
entire dataset.
}
...
with the package, they are described in Table @ref(tab:datasets-tb-pdf).
...
Another potential application of scagnostics is to detect shape
differences between groups. Commonly, classification focuses on
separations between groups or differences in the means, there are few
techniques that focus on difference in shape. This difference in shape
occurs when the variance patterns between groups are different. Linear
discriminant analysis (LDA) is an example of a method that does not take
this difference in variation into considering, quadratic discriminant
analysis (QDA), is an example of a method that does. LDA assumes the
distribution of each group is normal with the same variance, it then
draws a straight boundary that is at a maximal distance from each groups
mean. QDA assumes the distribution of each group is normal, and then
draws a curved boundary between them that is furthest from each groups
mean, but also respects that one group might have a larger elliptical
variance-covariance than another group. So from this, we can see that
QDA is more flexible than LDA as it allows for this difference in shape,
however the technique is still limited by an assumption of normality.
Scagnostics could be utilised in a similar fashion to QDA to identify
irregular shape differences between groups.
...
significantly smaller than the list of all possible scatter plots. Table
@ref(tab:scagsmacmic-pdf) shows the pair of features that maximises the
difference between groups for each scagnostic. Plotting a handful of
these in (Figure @ref(fig:timeseries)), we can see the difference in
shape that the scagnostics have identified. For example, the middle plot
of the figure shows the shape difference between the curvature and trend
strength between macroeconomic and microeconomic time series. Both types
of time series have, on average, strong trends and moderate curvature,
however the former varies more in the macroeconomic series and the later
in the microeconomic series. We can see from this example, and the other
...
technique based on scagnostics is beyond the scope of this research.

\begin{Schunk}
\begin{table}

\caption{(\#tab:scagsmacmic-pdf)Variable pairs that maximise between group difference of each scagnostic}
\centering
...
\begin{Schunk}
\begin{figure}
\includegraphics[width=0.5\linewidth]{mason-lee-laa-cook_files/figure-latex/wbistatic-1} \caption[The range of scagnostics calculated on data with a large number of variables can help to inform the analyst about the types of relationships present]{The range of scagnostics calculated on data with a large number of variables can help to inform the analyst about the types of relationships present. The side-by-side dotplot (left) shows one point for each pair of variables, with its highest scagnostic value among all scagnostics calculated. Most of the pairs of indicators exhibit outliers, skewed, stringy or convex. There is one pair that has clumpy as the highest value. The plots middle and right show the pair of variables with highest value on clumpy2 and convex respectively.}(\#fig:wbistatic)
\end{figure}
\end{Schunk}
...
plots, and by building upon previous work, we have implemented working
scagnostics calculations into the \emph{cassowaryr} package, without a
pre-processing binning step. We showed that the package works according
to definitions before presenting four possible applications of the
package. The first, AFLW was designed to show the general use of the
\emph{cassowaryr} package, and show how to best use scagnostics to find
unique scatter plots. In this example we also showed that looking at
specific pairwise scatter plots can give us valuable information about
our dataset, namely by interpreting the \emph{hitouts vs bounces}
scatter plot in Figure @ref(fig:threeplot-static) . Using simulated data
of a black hole merger event, we then displayed the packages ability to
identify pairwise non-linear relationships with convex, skinny, splines,
and dcor, as well as the improvement clumpy2 made in identifying
clustering. The time series example displayed the packages value as a
precursor to a new method of classification, where it successfully
identified scatter plots where the macro and micro economic time series
had different shape. Finally the scagnostics were applied to the World
Bank indicators to show how the scagnostics can be used to product an
...
the features data may lead us to believe that the maximum outlying value
...
research. \citet{hidscags} showed that scagnostics can be used to
identify hidden structure in pairwise plots after transformations such
as a log of the original data. This offers natural extension for the
scagnostics in the original code, but could also be considered a natural
extension of the \emph{cassowaryr} package. Finally, there are a handful
of scagnostics that are used to optimise the projection pursuit in the
\texttt{tourr} package and the scagnostics described here could also be
utilised there. Previously scagnostics used in projection pursuit had
has a large amount of noise, so the \texttt{tourr} struggles to
consistently move in a direction that will have a reliable increase in
the scagnostics. Checking if the adjusted scagnostic measures maintain
that issue, and further developing them if they arise, is another
possible area of future research. It is clear from this list that there
is a significant amount of research that could be built upon
scagnostics.
