...
\abstract{%
Data sets that have a large number of numeric vairbales, also have an
arbitrarily large number of possible pairwise scatter plots. Looking at
them all is infeasible for any individual, and so in order give priority
to the most interesting plots, we have designed the ``cassowaryr''
package that allows users to sort scatter plots based on a list of
Scagnsotics. Scagnsotics are scatter plot diagnostics that give
numerical summaries to the shapes of scatter plots. These scagnsotics
are shown to correctly order simulated data that represent scatter plot
shapes a user would find interesting. They are also applied to examples
in macro and micro time series, physics and sports data where the
package succeeds in picking out interesting scatter plots from a large
collection.
}
...
\emph{Improving the robustness of scagnsotics} \citep{robust}, and has
been included in the package. This measure is stronger than the original
clumpy measure as it builds a cluster hierarchy and considers a large
number of edges. Ultimately it seems to suffer from similar issues to
the original version of the scagnostic, and takes a significantly longer
time to compute than the original clumpy measure. It is not useful in
large scale scagnostic calculations (such as those done on every
pairwise combination of variables as is intended by the package) so is
not discussed here. The issues that need to be addressed with the new
clumpy measure are:
...
the mean of the large edges and the maximum of the small edges.

\emph{To add in after it is in the package version of clumpy\_adjusted:}
One we have identifies the large edges, we remove them from the MST and
identify which cluster all the inter cluster edges belong to. From each
cluster we identify the largest edge (to avoid the measure being
arbitrarily from small edges in the MST) and calculate the ratio between
the largest inter cluster edge, and the large edge that connects the
clusters. These ratios are added together as a weighted mean, where each
weight is determined by the number of points that do not satisfy the 180
degree striated requirement. This weighted mean is the final
clumpy\_adjusted value.
...
data set can be found in the appendix. With 38 variables, there are 703
possible scatterplots to make. The scagnostics can suggest which are the
interesting ones to examine.

\hypertarget{team-comparison}{%
\subsubsection{Team Comparison}\label{team-comparison}}

One use of scagnostics is to compare between group differences in the
shape of scatter plots. Here, we will apply the scagnsotics to the best
and worst team in the 2020 AFLW season, North Melbourne and Richmond
respectively. Looking at the \emph{shape} differences on scatter plots
between the two teams can give us a new method to understand what
differentiates a good team from a bad one.

After averaging each players statistics across all the games in the
season, there are 3 scatter plots that stand out as having a difference
in the shapes between the two teams. They are:
...
\hypertarget{turnovers-vs-disposal-efficiency}{%
\paragraph{Turnovers vs Disposal
Efficiency}\label{turnovers-vs-disposal-efficiency}}

\begin{Schunk}

\includegraphics{mason-lee-laa-cook_files/figure-latex/unnamed-chunk-9-1} \end{Schunk}

This scatter plot was identified as interesting as the two teams has a
large difference on the clumpy\_adjusted and convex measures. A good
player will have a high disposalEfficiency (they rarely kick it to the
other team) and a low number of turnovers (they rarely kick it somewhere
no player can get it) and we would expect a linear relationship between
the two, as causing a both is likely caused by bad play. A comparison of
these plots shows the best team in the league, North Melbourne, adheres
pretty well to this linear relation, while Richmond does not. That
indicates that Richmond players either overwhemingly kick the ball into
space to avoid kicking to the other team, or they overwhelmingly give a
risky pass that is likely to go to the other team. They do not evenly
choose between the two.

\hypertarget{one-percenters-vs-inside-50s}{%
\paragraph{One Percenters vs Inside
50s}\label{one-percenters-vs-inside-50s}}

\begin{Schunk}

\includegraphics{mason-lee-laa-cook_files/figure-latex/unnamed-chunk-10-1} \end{Schunk}

This scatter plot was also identified as interesting by the two teams
having a large difference on the clumpy\_adjusted and convex measures.
If the player gets the ball inside the 50m arc around the oppositions
goal they are likely a forward, if they get perform a lot of
onePercenters they are likely a defensive player, if they do a bit of
both they are likely a midfielder. A well structured team will have a
lower triangular shape on this scatter plot, as players are sticking to
their position and excelling in that position, which is the structure
North Melbourne has. Richmond has several players that score highly on
both onePercenters and are frequently inside50s. This implies Richmond
has a hanful of players that play every position well and are a bit all
over the field, and have less specialisation in positions.

\hypertarget{clangers-vs-handballs}{%
\paragraph{Clangers vs Handballs}\label{clangers-vs-handballs}}

\begin{Schunk}

\includegraphics{mason-lee-laa-cook_files/figure-latex/unnamed-chunk-11-1} \end{Schunk}

This scatter plot was identified as interesting by all the association
measures. Handballs are a measure of one of the legal methods of getting
rid of the ball, clangers are a measure of the illegal methods of
getting rid of the ball. If the number of clangers for each player in
the team linearly related to the number of handballs, it means the team
as a whole is efficient at getting rid of the ball, if this scatter plot
is spread out, it means there are large gaps in the efficiency of each
player. This scatter plot shows us that the Richmond team has a greater
difference in ability than the North Melbourne Team.

\hypertarget{a-look-at-splines}{%
\subsubsection{A Look at Splines}\label{a-look-at-splines}}
...
\hypertarget{stirated-adjusted-comments}{%
\subsubsection{Stirated Adjusted
Comments}\label{stirated-adjusted-comments}}
...
\includegraphics{mason-lee-laa-cook_files/figure-latex/unnamed-chunk-15-1} 
\includegraphics{mason-lee-laa-cook_files/figure-latex/unnamed-chunk-15-2} \end{Schunk}
...
goal and dispsoal accuracy vs number.
