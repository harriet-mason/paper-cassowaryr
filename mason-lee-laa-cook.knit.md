...
abstract: >
  As the number of dimensions in a data set increases, the process of visualising its structure and variable dependencies becomes more tedious. Scagnostics (scatterplot diagnostics) are a set of numerical measures describing visual features that can be used to identify interesting and abnormal scatterplots, and give a sense of priority to the variables we choose to visualise. A new set of scagnostics, containing previously defined measures and newly created measures, are implemented in the cassowaryr R package, which provides a user-friendly method to apply scagnostics to data in R. After implementing the scagnostics from the original defnitions we found that some measures did not work as expected, and variations were developed. The application of scagnostics is illustrated with examples on sport statistics and astrophysics, that show they can be useful for exploratory data analysis, identifying shape differences between groups, and as a summary tool for the shape of multivariate data.
date: "2025-12-16"
author:  
...
\begin{figure}
\includegraphics[width=1\linewidth,alt={A hand drawn illustration of a scatter plot with its respective convex hull, alpha hull, and minimum spanning tree.}]{mason-lee-laa-cook_files/figure-latex/building-blocks2-1} \caption{The building blocks for graph-based scagnostics: (a) convex hull, (b) alpha hull and (c) minimal spanning tree. The convex hull is a convex shell around all the data points. The alpha hull contains all the points but allows concavities better capturing some shapes, but it needs tuning. The minimal spanning tree connects all points once, and has a single chain connecting central points.}(\#fig:building-blocks2)
\end{figure}
...
Before any of the scagnostics are calculated, the data goes through a pre-processing step where outlying points are removed. Outliers are defined as any point whose adjacent edges in the MST have edges larger $q_{75} + 1.5(q_{75} - q_{25})$, where $q_i$ refers to the i$^{th}$ percentile of the sorted edge lengths of the MST. By default, all the scagnostics are calculated on the processed data, excluding *outlying* as it compares the MST of the raw data to the MST of the processed data. However, this is not set in stone, and the package offers the option to calculate scagnostics on the orignal data if the user prefers. 
...
\begin{figure}
\includegraphics[width=1\linewidth,alt={Fifteen scatter plots of the features data where each scatter plot has a distinct visual shape.}]{mason-lee-laa-cook_files/figure-latex/features-plot-1} \caption{The scatter plots of the features dataset. These scatter plots were designed to each represent a distinct visual feature, for example the ring scatter plot is a hollow version of disk. The scagnostics need to be able to differentiate these plots.}(\#fig:features-plot)
\end{figure}
...
Skewed & This measure can identify skewed edge lengths, such as the L shape in the visual table, that typically require a log transformation. The skewed value rarely drops below 0.5 or rises above 0.8. Skewed seems to suffers from a similar issue to sparse regarding binning.\\
...
\begin{figure}
\includegraphics[width=1\linewidth,alt={A visual table of the feature scatter plots on the striated and striated 2 measures where striated 2, unlike striated, gives a value of 1 to a plots that is continuous in one axis and discrete on the other.}]{mason-lee-laa-cook_files/figure-latex/striated-vtable-1} \caption{Using a visual table to compare the striated and it's adjusted counterpart, striated2 allows us to visualise the difference in the measures. While the functions may seem similar at a glance, striated2 has a stricter version of discreteness, hence why line and vlines have the same result and plots with no discreteness score a 0.}(\#fig:striated-vtable)
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth,alt={A visual table of the feature scatter plots on the cluump and clumpy 2 measures where clumpy 2, unlike clumpy, orders the features plots according to how 'clustered' they are and actually ranges from 0 to 1.}]{mason-lee-laa-cook_files/figure-latex/clumpy-vtable-1} \caption{A visual table comparing the scagnostic values of clumpy and clumpy2. We can see the clusters plot is next to last in the ordering of the original clumpy measure, but first in clumpy2. It is clear that clumpy2 achieves a more ballanced distribution and more intuitive plot ordering.}(\#fig:clumpy-vtable)
\end{figure}
...
sc\_clumpy2 & Compute adjusted clumpy scagnostic measure.\\
...
sc\_striated & Compute the original striated scagnostic measure.\\
...
\begin{figure}
\includegraphics[width=1\linewidth,alt={A scatter plot of simulated organised data where there are multiple ways of drawing the minimum spanning tree.}]{mason-lee-laa-cook_files/figure-latex/outlying-test-plot-1} \caption{Plot of simulated data used for testing the outlying scagnostic. The left plot shows the raw data, while the right plot presents the MST generated on that data. When we created the test we expected the red dashed line to be in the MST, but instead the green line that connects points 3 and 4 is. If the red edge is in the MST rather than the green edge, the outlying value on this plot is much higher.}(\#fig:outlying-test-plot)
\end{figure}
...
To pick out which pair wise plots might be interesting to examine, we use a SPLOM of scagnostic values. Technically, any high-dimensional visualisation approach, such as a parallel co-ordinate plot, can be utilised to find interesting pairs, but we opted to use a SPLOM as it is the approach taken by @scagdist. When looking at the SPLOM, the points that stick to the main mass and have standard values on all the scagnostics are exemplar plots for this data set, while unusual plots that require further inspection will appear as outliers. We used a SPLOM to identify these plots for the AFLW data. 
...
{\centering \includegraphics[width=1\linewidth,alt={Six scatter plots of AFL sports statistics where all six scatter plots are clearly visually distinct from one another.}]{mason-lee-laa-cook_files/figure-latex/AFLW-scatters-static-1} 
...
Figure \@ref(fig:wbistatic) (left plot) shows a summary of the top scagnostic values. This is displayed as a vertically-oriented side-by-side dotplot. Missing from the plot, but not the calculations, is that splines was not the high for any pair of variables in the data set. The other two plots show the pairs of variables with high values on outlying.
<!-- and striped were not highest for any pair of variables in the data set. -->
Generally, scagnostics might be useful for obtaining alternative descriptive summaries of data with many variables. Primarily, they guide the choice of individual scatter plots to make. 
